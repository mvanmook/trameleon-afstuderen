/*
  ...
  GENERAL INFO:
    ...
    This file creates an instance of `netintdec` and decodes
...
int main(int argc, const char* argv[]) {
  ...
  const char* input_filename = "./test-module-netint-encoder.h264";
  ...
  dec_opt.output_type = TRA_MEMORY_TYPE_H264;
  dec_opt.bitrate = 700e3;
  dec_opt.fps_num = 25;
  dec_opt.fps_den = 1;

  r = tra_core_decoder_create(core, "nidec", &dec_opt, NULL, &dec);
  if (r < 0) {
    r = -40;
    goto error;
  }
...
static int on_decoded_data(uint32_t type, void* data, void* user) {
  ...
  if (TRA_IMAGE_FORMAT_I420 != img->image_format) {
    TRAE("Received decoded data, but we're not writing it to a file as we only support I420, we received: %s", tra_imageformat_to_string(img->image_format));
    r = -20;
    goto error;
  }
